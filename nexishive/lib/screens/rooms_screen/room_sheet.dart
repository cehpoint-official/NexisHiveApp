...
import 'package:untitled/models/verification.dart';
...
class RoomSheet extends StatelessWidget {
  RoomSheet({Key? key, this.isFromInfo = false, required this.room, required this.controller}) : super(key: key);
  final bool isFromInfo;
  final Room room;
  final RoomController controller;
  final VerifyUser verifyUser = VerifyUser();

  void showQuestionDialog(BuildContext context) async {
    String? question = await verifyUser.questionGenerator(room.title ?? 'general');
    ...
    if (question != null && question.isNotEmpty) {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return WillPopScope(
            onWillPop: () async {
              Get.snackbar("Warning", "Don't change the screen while answering");
              return false;
            },
            child: AlertDialog(
              title: Text("Answer the question to join"),
              content: Column(
              ...
              actions: [
                ...
                TextButton(
                  ...
                  onPressed: () async {
                    ...
                    if (answer.isEmpty) {
                      Get.snackbar("Error", "Please provide an answer.");
                      return;
                    }

                    String? verification = await verifyUser.isCorrect(answer, question);

                    if (verification != null && verification.contains("yes")) {
                      Get.back();
                      Get.snackbar("Success", "You are eligible for this group.");
                      controller.joinOrRequestOrAccept();
                    } else {
                      Get.back();
                      Get.snackbar("Error", "You are not eligible for this room.");
                    }
                  },
                ),
              ],
      ...
      Get.snackbar("Error", "Unable to generate question. Please try again.");
    }
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      isFromInfo
                          ? Container()
                          : CommonSheetButton(
                              color: controller.room.getUserAccessType() == GroupUserAccessType.requested
                              ...
                              title: room.getUserAccessType() == GroupUserAccessType.invited
                                  ...
                                  : room.isJoinRequestEnable == 1
                                      ? controller.room.getUserAccessType() == GroupUserAccessType.requested
                                          ...
                                          : LKeys.requestToJoin
                                      : LKeys.joinThisRoom,
                              onTap: () => showQuestionDialog(context),
                            )
                    ],
                  ),
                ),
                ...
                return Positioned(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                        border: Border.all(color: cWhite.withOpacity(0.8), width: 2), shape: BoxShape.circle),
