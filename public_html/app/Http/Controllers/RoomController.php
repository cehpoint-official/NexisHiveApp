...
class RoomController extends Controller
{
    ...
    public function acceptInvitation(Request $request)
    {
        ...
        if ($acceptRequest) {
            $roomUser = RoomUser::where('id', $acceptRequest->id)->first();
            ...
            if ($acceptRequest->type == 1 || $acceptRequest->type == 4) {
                ...
                if ($user->id != $roomUser->invited_by) {
                    if($user->is_push_notifications == 1) {
    ...
    {
        $report = Report::where('id', $request->report_RoomId)
            ->get()
            ->first();

        if ($report) {
            $roomUsers = RoomUser::where('room_id', $report->room_id)->get();
            $roomUsers->each->delete();

            $room = Room::where('id', $report->room_id)
                ->get()
                ->first();
            $room->delete();
    ...
    public function deleteRoom(Request $request)
    {
        ...
        if ($roomUser) {
            $roomUserDelete = RoomUser::where('room_id', $request->room_id)->get();
            $roomUserDelete->each->delete();

            $room = Room::where('id', $request->room_id)->first();
            $room->delete();

            $userNotification = SavedNotification::where('room_id', $request->room_id)->get();
    ...
    public function roomsListWeb(Request $request)
    {
        ...
        foreach ($result as $item) {
            ...
            $imageUrl = GlobalFunction::createMediaUrl($item->photo);
    ...
    public function userRoomInTable(Request $request)
    {
        ...
        foreach ($result as $item) {
            ...
            $imageUrl = GlobalFunction::createMediaUrl($item->room->photo);
            ...
            $view = '<a href="../roomDetails/' . $item->room->id . '" data-title="' . $item->title . '" class="ms-3 btn btn-info px-4 text-white edit" rel=' . $item->id . ' data-tooltip="View User">' . __('<svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg> <span class="ms-2"> View </span>') . '</a>';
            $action = '<span class="float-right">' .  $view . ' </span>';
    ...
    public function deleteThisRoom(Request $request)
    {
        ...
        if ($room) {
            ...
            $roomReports = Report::where('room_id', $room->room_id)->get();
            $roomReports->each->delete();

            $roomUser = RoomUser::where('room_id', $request->room_id)->get();
            $roomUser->each->delete();
